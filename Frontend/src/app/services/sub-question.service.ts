...
export class SubQuestionService {
  private apiUrl = `${baseUrl}/subQuestions`;
  ...
  addSubQuestion(subQuestion: any): Observable<any> {
    ...
    return this.http.post<any>(`${this.apiUrl}/add`, subQuestion,{headers});
  }

  updateSubQuestion(id: number, subQuestion: any): Observable<any> {
    const token = localStorage.getItem('token');
    const headers = new HttpHeaders().set('Authorization', `Bearer ${token}`);
    return this.http.put<any>(`${this.apiUrl}/update/${id}`, subQuestion,{headers});
  }
  ...
  getSubQuestionsOfParentQuestion(parentId: number): Observable<any[]> {
    const token = localStorage.getItem('token');
    const headers = new HttpHeaders().set('Authorization', `Bearer ${token}`);
    return this.http.get<any[]>(`${this.apiUrl}/parentQuestion/${parentId}`,{headers});
  }
}